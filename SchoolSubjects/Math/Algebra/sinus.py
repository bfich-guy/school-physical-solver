...
def get_angle_sinus_by_degrees(*,
                               degrees: str,
                               ) -> Decimal | None:
    ...
    try:
        ...
        angle_sinus: Decimal = Decimal(format(Decimal(str(sin(radians(float(decimal_degrees))))).quantize(DECIMAL_QUANTIZE_TARGET).normalize(), DECIMAL_FORMAT_MODE))
        return angle_sinus
