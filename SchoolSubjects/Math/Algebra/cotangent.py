...
def get_angle_cotangent_by_degrees(*,
                                   degrees: str,
                                   ) -> Decimal | None:
    ...
    try:
        ...
        angle_cotangent: Decimal = Decimal(format(Decimal(str(1 / tan(radians(float(decimal_degrees))))).quantize(DECIMAL_QUANTIZE_TARGET).normalize(), DECIMAL_FORMAT_MODE))
