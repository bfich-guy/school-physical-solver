...
def get_angle_cosinus_by_degrees(*,
                                 degrees: str,
                                 ) -> Decimal | None:
    ...
    try:
        ...
        angle_cosinus: Decimal = Decimal(format(Decimal(str(cos(radians(float(decimal_degrees))))).quantize(DECIMAL_QUANTIZE_TARGET).normalize(), DECIMAL_FORMAT_MODE))
